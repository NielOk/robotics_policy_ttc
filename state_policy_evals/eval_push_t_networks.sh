...
if [[ $run_inference == "y" ]]; then
    ...
    echo "Running eval script on remote instance..."
    ssh -i "$private_ssh_key" "$remote_ssh_user@$remote_ssh_host" "source .venv/bin/activate && nohup python3 ~/$EVAL_SCRIPT_PATH > push_t_state_policy_evals.log 2>&1 &" &
else
