...
cd push_t_vision_policy_experiments/
...
MAIN_SCRIPT_PATH="./vision_policy_push_t.py"
DATASET_SCRIPT_PATH="./push_t_vision_dataset.py"
ENV_SCRIPT_PATH="./push_t_vision_env.py"
NETWORK_SCRIPT_PATH="./push_t_vision_network.py"
ENCODER_SCRIPT_PATH="./push_t_vision_encoder.py"
...
if [[ $copy_script == "y" ]]; then
    ...
    scp -i "$private_ssh_key" "$ENCODER_SCRIPT_PATH" "$remote_ssh_user@$remote_ssh_host:~/$ENCODER_SCRIPT_PATH"
else
...
if [[ $run_inference == "y" ]]; then
    ...
    ssh -i "$private_ssh_key" "$remote_ssh_user@$remote_ssh_host" "source .venv/bin/activate && nohup python3 ~/$MAIN_SCRIPT_PATH > push_t_vision_policy_experiment_results.log 2>&1 &" &
else
